...
def procesar_marcaciones(fecha_inicio=None, fecha_fin=None):
    if not fecha_inicio or not fecha_fin:
        ultimo_registro = EmpleadoTurno.objects.order_by("-fecha").first()
        if ultimo_registro:
            fecha_inicio = ultimo_registro.fecha + timedelta(days=1)
        else:
    ...
    for empleado in empleados:
        ...
        marcaciones = AttPunch.objects.filter(
            employee=empleado,
            punch_time__date__range=[fecha_inicio - timedelta(days=1), fecha_fin + timedelta(days=1)]
        ).order_by('punch_time')
        ...
        for fecha, marcas in dias.items():
            ...
            if nombre_turno == "Turno 1" and grupo in GRUPOS_TURNO_1:
                ...
                if entrada.time() < turno["hora_entrada_min"]:
                    tiempo_extra = (datetime.combine(fecha, turno["hora_entrada_min"]) - datetime.combine(fecha, entrada.time())).total_seconds() / 3600
                    horas_extras_diurnas = min(tiempo_extra, max(turno.get("rangos_horas_extra_diurna", [0])))
                    datos_base["horas_extras_diurnas"] = horas_extras_diurnas if not festivo else 0
                    datos_base["horas_extras_festivas_diurnas"] = horas_extras_diurnas if festivo else 0
                if time(21, 0) <= salida.time() <= time(22, 0):
                    datos_base["recargo_nocturno"] = 1 if not festivo else 0
                    datos_base["recargo_nocturno_festivo"] = 1 if festivo else 0
